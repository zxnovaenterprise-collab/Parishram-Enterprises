...
export const SalarySlipModal: React.FC<SalarySlipModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col h-screen bg-slate-950/80 backdrop-blur-md print:static print:bg-white print:h-auto print:block">
      
      {/* Top Header Bar */}
      <div className="bg-slate-900 text-white px-6 py-3.5 flex items-center justify-between shrink-0 print:hidden border-b border-slate-800 shadow-lg z-30">
        <div className="flex items-center gap-2">
          <ShieldCheck className="w-5 h-5 text-emerald-400" />
          <h3 className="text-sm font-bold">Salary Slip Preview: {record.name} ({record.monthYear})</h3>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold rounded-xl shadow cursor-pointer transition-all active:scale-95"
          >
            <X className="w-4 h-4" />
            Close
          </button>
      ...
      {/* Middle Scrollable Payslip Container */}
      <div className="flex-1 overflow-y-auto p-4 sm:p-8 modal-scroll-area print:p-0 print:overflow-visible">
        <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full mx-auto border border-slate-200 overflow-hidden print:shadow-none print:border-none print:max-w-none print:w-full">
          ...
          <div className="p-8 text-slate-800 font-sans print:p-6" id="printable-payslip">
            ...
            <div className="pt-12 grid grid-cols-2 gap-8 text-center text-xs text-slate-500">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">{settings.signatoryName || 'For Parishram Enterprises'}</p>
              </div>
            </div>
      ...
      {/* Modal Action Footer */}
      <div className="bg-slate-900 px-6 py-3 border-t border-slate-800 flex items-center justify-between shrink-0 print:hidden text-white z-30">
        <span className="text-xs text-slate-300 font-medium">Official Payslip Document Preview</span>
        <div className="flex items-center gap-3">
          <button
            onClick={handlePrint}
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-5 py-2 bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold rounded-xl shadow border border-slate-700 cursor-pointer transition-all"
          >
            <X className="w-4 h-4" />
            Close Preview
          </button>
